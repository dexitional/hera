...
const electionsQueryOptions = (electionId: any) => ({
  ...
  queryFn: () => getUnifiedElectionAdminStats({ data: electionId }),
  refetchInterval: 30 * 1000,
  staleTime: 30 * 1000,
  refetchIntervalInBackground: true,
});

export const Route = createFileRoute("/admin/elections/$electionId/adminfeed")({
    ...
    <div className="max-w-7xl mx-auto space-y-6 p-4 md:p-6 text-zinc-200 font-sans select-none overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0a192a]/50 p-6 rounded-xl border border-zinc-800">
        <div>
          <div className="flex items-center gap-2 text-xs text-zinc-500 font-mono uppercase tracking-wider">
            ...
            <span className="text-zinc-400 select-all">
              Live Stream Metrics Feed
            </span>
          </div>
          <h1 className="text-xl font-bold text-white tracking-tight flex items-center gap-2 mt-1">
            <Activity
              className={`w-5 h-5 text-purple-400 \${isSyncing ? 'animate-pulse text-purple-300' : ''}`}
            />
        ...
        <div className="inline-flex items-center gap-2 text-xs bg-zinc-900 border border-zinc-800 px-3.5 py-1.5 rounded-lg select-none shrink-0 font-medium">
          <RefreshCw
            className={`w-3.5 h-3.5 text-purple-400 \${isSyncing ? 'animate-spin' : ''}`}
          />
        ...
        <div className="bg-[#0a192a]/50 border border-zinc-800 p-4 rounded-xl flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-black text-white mt-1 font-mono">
              {!isNaN(parseFloat(turnoutPercentage))
                ? turnoutPercentage
                : "0.0"}
              %
            </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          {ballotTally?.map((position: any) => (
            <div
              key={position.id}
              className="bg-[#0a192a]/50 border border-zinc-800 rounded-xl p-5 space-y-4 shadow-xl"
            >
              ...
              <div className="space-y-4 pt-1">
                {position?.candidates?.map((candidate: any) => {
                  const isAbstain =
                    candidate.id === null || candidate.id === -1;
                  const percentageShares =
                    position.totalVotesForPosition > 0
                      ? (
                          (candidate.votes / position.totalVotesForPosition) *
                          100
                        ).toFixed(1)
                      : "0.0";
                  const clampedWidth = Math.min(
                    parseFloat(percentageShares),
                    100,
                  );

                  return (
                    <div
                      key={candidate.id ?? `abstain-\${position.id}`}
                      className="space-y-2"
                    >
                      <div className="flex items-center justify-between text-xs font-medium">
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div
                            className={`w-7 h-7 rounded border overflow-hidden flex items-center justify-center shrink-0 \${
                          ...
                          <span
                            className={`font-semibold truncate max-w-[180px] md:max-w-xs \${
                              isAbstain ? 'text-amber-400' : 'text-zinc-200'
                            }`}
                          >
                            {isAbstain
                              ? "Abstained (Blank Ballots)"
                              : candidate.name}
                          </span>
                        </div>
                        ...
                        <div className="text-right shrink-0 font-mono text-[11px] font-bold text-zinc-400">
                          ...
                          <span className="text-zinc-600 font-normal px-1">
                            ·
                          </span>
                          <span
                            className={
                              isAbstain ? "text-amber-400" : "text-purple-400"
                            }
                          >
                            {percentageShares}%
                          </span>
        ...
        <div className="bg-[#0a192a]/50 border border-zinc-800 rounded-xl p-5 space-y-4 shadow-xl">
          ...
          <p className="text-[11px] text-zinc-500 leading-relaxed">
            Cryptographically timestamped transaction streams indicating
            individual ballot drop channels across server verification queues.
          </p>
              ...
              >
                <div className="min-w-0 space-y-1">
                  ...
                  <div className="text-[10px] text-zinc-500 font-sans font-medium flex items-center gap-1">
                    <span>Mask:</span>
                    <span className="text-zinc-400 font-mono font-bold select-all">
                      {log.voterMask}
                    </span>
                  </div>
                </div>
                ...
                <div className="text-right shrink-0 flex flex-col items-end gap-1 font-sans">
                  <span
                    className={`text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded border select-none ${log.channel === "USSD" ? "border-amber-900/30 text-amber-400 bg-amber-950/20" : "border-blue-900/30 text-blue-400 bg-blue-950/20"}`}
                  >
                    {log.channel}
                  </span>
          ...
          <div className="bg-zinc-900/40 border border-zinc-800 p-3 rounded-lg text-[10px] text-zinc-400 flex items-start gap-2 leading-relaxed">
            ...
            <span>
              To protect secret ballot rights, voter telephone links undergo
              hardware hashing methods inside cryptographic index files.
            </span>
          </div>
        </div>
      </div>
