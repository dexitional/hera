...
function PositionsDirectory() {
  ...
  const uniqueElections = ["ALL", ...Array.from(new Set(positions.map((p:any) => p.electionTitle)))];
  ...
  const filteredPositions = positions.filter((position: any) => {
    ...
    const matchesFilter = selectedElectionFilter === "ALL" || position?.electionTitle === selectedElectionFilter;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0a192a]/50 p-6 rounded-xl border border-zinc-800">
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight">Position Manager</h1>
            <p className="text-xs text-zinc-400 mt-1">
              Configure corporate offices, establish winner slot limits, and organize your digital ballot papers.
            </p>
          </div>
          ...
          <div className="relative w-full sm:w-auto" ref={dropdownRef}>
            ...
            >
              <div className="flex items-center gap-2 truncate">
                ...
                <span className="text-white truncate">{selectedElectionFilter}</span>
              </div>
              ...
              <tbody className="divide-y divide-zinc-900 text-sm">
                {filteredPositions.length > 0 ? (
                  filteredPositions.map((position: any) => (
                    <tr key={position.id} className="hover:bg-zinc-900/20 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="font-semibold text-white tracking-wide">{position.title}</span>
                      ...
                      <td className="px-4 py-4 align-middle text-center">
                       ...
                       <td className="px-4 py-4 align-middle text-center">
                        <div className="inline-flex items-center gap-1 text-xs font-mono font-bold bg-zinc-900 border border-zinc-800 text-zinc-300 px-2.5 py-1 rounded shadow-inner">
                          <span className="text-[10px] text-zinc-500 font-sans font-normal ml-1">Order</span>
                          <Hash className="w-3 h-3 text-zinc-500" />
                      ...
                      <td className="px-6 py-4 align-middle text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            to={`/admin/positions/${position?.id}/edit`}
